with raw_qa_rvc_we as (
    ...
    where date(cast(cast(tanggal_selesai_verifikasi as timestamp) as date)) >= current_date - interval '5' day
        and date(cast(cast(tanggal_selesai_verifikasi as timestamp) as date)) <= current_date - interval '3' day
...
ranked_data as (
    select 
        *, 
        row_number() over (partition by agent order by no_pengajuan desc) as rn
    from raw_qa_rvc_we
),
...
data_qa as (
    ...
    where rn <= (
                ...
                where sub.agent = ranked_data.agent)
),
...
max_row_cte AS (
    SELECT MAX(no) AS max_row_value
    FROM qa_sample
)
...
from qa_sample
where no < (
            select MAX(no) from qa_sample)
    and (select max_row_value from max_row_cte) % 2 <> 0
order by 1 asc
