...
where substring(date_assign,1,10) = date_format(now(),'%Y-%m-%d')
    ...
    and Analyst in ('allan.solichin','agustin.putri','Irma suryani nasution','faizzain.muhammad',
                    'saifullah.saputra','arief','adi.tri','mauren.olivia','Nur.ismail','achmad',
                    'dyonisia.anggita','alisa','achmad.rivai','dessy.astarini','hadi.putra','lutfi.fanani')
group by 1,2
order by 1,2